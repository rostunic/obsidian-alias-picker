import { App, TFile, CachedMetadata, LinkCache, Notice } from 'obsidian';
...
export class AliasRenameListener {
	...
	public stopListening() {
		this.app.metadataCache.off('changed', this.onMetadataChanged as any);
	}
	private onMetadataChanged = async (file: TFile, _data: string, cache: CachedMetadata) => {
		...
		if (addedAliases.length === 1 && removedAliases.length === 1) {
			...
			const backlinksArray = this.getBacklinksArray(file);
    ...
    public getBacklinksArray(file: TFile) {
        const backlinksObject = (this.app.metadataCache as any).getBacklinksForFile(file) as { data: Map<string, LinkCache[]>; };
        const backlinksArray = Array.from(backlinksObject.data.entries());
